...
int main(){
    ...
    std::string inputfile = base + "buddha.obj";
    ...
    if(textures.empty()) textures.push_back(loadTexture2D(base + "tiger-atlas.jpg"));
    ...
    glfwSetWindowUserPointer(win, &cam);
        ...
        if(glfwGetMouseButton(win, GLFW_MOUSE_BUTTON_LEFT) == GLFW_PRESS) {
            ...
            if(!draggingModel) { draggingModel = true; lastx = x; }
            else {
                float dx = float(x - lastx);
                modelYaw += dx * 0.3f;   // 旋轉速度係數
                lastx = x;
            }
        } else draggingModel = false;
