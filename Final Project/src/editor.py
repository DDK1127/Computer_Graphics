...
def explode_body_geoms(root: ET.Element, src_body_name: str, prefix: str, keep_original: bool):
    ...
    # only direct-child geoms of this body (your file matches this)
    ...
    # Create a new <body> per geom
    for idx, g in enumerate(geoms):
        mesh = g.get("mesh")
        # Use mesh name if exists; otherwise use index.
        suffix = mesh if mesh else f"geom{idx}"
        new_body_name = f"{prefix}__{suffix}"

        # Ensure uniqueness by appending if needed
        base_name = new_body_name
        k = 1
        existing = set(list_named(root, "body"))
        while new_body_name in existing:
            k += 1
            new_body_name = f"{base_name}_{k}"

        nb = ET.SubElement(worldbody, "body")
        nb.set("name", new_body_name)
        # keep at origin; you can move it later via --move-body / --rot-body-*
        nb.set("pos", "0 0 0")
        nb.set("euler", "0 0 0")

        # copy geom (deep copy via tostring/fromstring)
    ...
    if not keep_original:
        # remove original geoms from src body
        for g in geoms:
            src_body.remove(g)
